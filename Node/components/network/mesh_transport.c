...
esp_err_t mesh_transport_send_to_root(const uint8_t *data, size_t length)
{
    ...
    if (err != ESP_OK)
    {
        ESP_LOGE(TRANS_TAG, "Failed to send upstream: %s", esp_err_to_name(err));
    }
    return err;
}
...
void mesh_transport_receive_task(void *param)
{
    ...
    while (1)
    {
        ...
        esp_err_t err = esp_mesh_recv(&from,
                                      &data,
                                      portMAX_DELAY,
                                      &flag,
                                      NULL,
        ...
        {
            ESP_LOGI(TRANS_TAG,
                     "Received msg type %u from %02X:%02X:%02X:%02X:%02X:%02X len %u",
                     packet.message_type,
        ...
        {
            ESP_LOGW(TRANS_TAG,
                     "Received undecodable packet size %u",
                     (unsigned)data.size);
        }
    }
}
